...
- **`pkg/vegetarun`** — load testing: `Run` converts the already-built `*http.Request` into a static vegeta target and attacks it per the `@vegeta` profile (rate/duration/workers/connections/max-body), returning metrics + a rendered text report. Gated by the `-vegeta` flag: `main.runFile` passes a nil `VegetaRunner` (`vegetarunner.go`) when the flag is off, so marked requests run as normal single requests. Attacked requests resolve vars and run the pre-script once; the post-script is skipped, and success < 100% sets `Result.Err` (exit 2). `Result.Vegeta` exempts the result from the `-strict` status check.
